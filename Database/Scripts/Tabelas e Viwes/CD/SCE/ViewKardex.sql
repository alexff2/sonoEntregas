...
/****** Object:  View [dbo].[VIEW_KARDEX_PRODUTOS]    Script Date: 03/10/2021 09:29:50 ******/
SET ANSI_NULLS OFF
GO
...
WHERE PROCESSAR = 'S'
UNION
SELECT 1 AS LOJA, C.D_DELIVERED AS DATA, CONVERT(VARCHAR(8), '00:00:00') AS HORA, CONVERT(VARCHAR(30), 'ENTREGA') AS MODULO, 
...
CONVERT(VARCHAR(30), 'Padrao') AS USUARIO, 1 AS ITEM, -1 AS MU_ITEM, 1 AS MU_QTE_MENORUN, CONVERT(CHAR(3), 'UN') MU_UNIDADE
	FROM PRODUTOS A
	INNER JOIN (SELECT A.COD_ORIGINAL, SUM(A.QUANTIDADE) QTD, C.D_DELIVERED, B.ID_SALES, B.NOMECLI, B.ID_CLIENT, A.UNITARIO1
				FROM SONOENTREGAS..SALES_PROD A
				INNER JOIN SONOENTREGAS..SALES B ON A.ID_SALES = B.ID_SALES
				INNER JOIN SONOENTREGAS..DELIVERYS_PROD C ON A.COD_ORIGINAL = C.COD_ORIGINAL AND A.CODLOJA = C.CODLOJA AND A.ID_SALES = C.ID_SALE
				WHERE C.DELIVERED = 0
				GROUP BY A.COD_ORIGINAL, C.D_DELIVERED, B.ID_SALES, A.CODPRODUTO, B.NOMECLI, B.ID_CLIENT, A.UNITARIO1) C 
