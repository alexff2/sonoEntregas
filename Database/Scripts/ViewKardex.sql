...
/****** Object:  View [dbo].[VIEW_KARDEX_PRODUTOS]    Script Date: 05/31/2021 08:32:44 ******/
SET ANSI_NULLS OFF
GO
...
ALTER VIEW [dbo].[VIEW_KARDEX_PRODUTOS](
 ...
 MU_UNIDADE
  ...
  AS
SELECT     N.CODLOJA AS LOJA, N.DATA_CHEGADA AS DATA, N.HORA_LANCAMENTO AS HORA, CONVERT(VARCHAR(30), 'NF ENTRADA') AS MODULO, CONVERT(CHAR(15), N.NUM_DOC) AS DOCUMENTO, 
					  I.PRODUTO AS CODPRODUTO, 
                      CASE 
...
WHERE PROCESSAR = 'S'
UNION
SELECT 1 AS LOJA, C.D_ENTREGA AS DATA, CONVERT(VARCHAR(8), '00:00:00') AS HORA, CONVERT(VARCHAR(30), 'ENTREGA') AS MODULO, 
...
CONVERT(VARCHAR(30), 'Padrao') AS USUARIO, 1 AS ITEM, -1 AS MU_ITEM, 1 AS MU_QTE_MENORUN, CONVERT(CHAR(3), 'UN') MU_UNIDADE
	FROM PRODUTOS A
	INNER JOIN (SELECT A.COD_ORIGINAL, SUM(A.QUANTIDADE) QTD, B.D_ENTREGA, B.ID_SALES, B.NOMECLI, B.ID_CLIENT, A.UNITARIO1
				FROM SONOENTREGAS..SALES_PROD A
				INNER JOIN SONOENTREGAS..SALES B ON A.ID_SALES = B.ID_SALES
				GROUP BY A.COD_ORIGINAL, B.D_ENTREGA, B.ID_SALES, A.CODPRODUTO, B.NOMECLI, B.ID_CLIENT, A.UNITARIO1) C 
