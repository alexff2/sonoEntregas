...
export default function ModalAddSale({ setOpen, forecast }){
  ...
  const handleSearchSales = async () => {
    try {
      ...
      let { data }  = await api.get(`sales/${idSale}/forecast/create`)
  ...
  const addSalesInData = async () => {
    try {
      ...
      if(saleFiltered.length === 0) {
        setErrorMsg('Selecione ao menos um produto do DAV!')
        return
      }
  ...
  return(
    <Box component={Paper} width={'1100px'} p={4}>
      <Typography
        variant='h4'
        style={{ marginBottom: 12 }}
        component='h2'
      >Adicionar venda</Typography>
